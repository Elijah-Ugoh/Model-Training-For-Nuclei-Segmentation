...
### Nuclei Detection Uisin the Trained Model
...
1. With the instructions on this page, the trained [StarDist 2D model is incorporated  2D directly within QuPath](https://qupath.readthedocs.io/en/0.4/docs/deep/stardist.html). 

   - When the [QuPath extension for StarDist](https://github.com/qupath/qupath-extension-stardist) is downloaded, the path for the custom trained model must be changed. 
...
2. To use the trained model in Fiji, the instructions provided [here](https://imagej.net/plugins/stardist) must be followed to import the model into Fiji's StarDist 2D model. 
