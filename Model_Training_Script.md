...
# Finally, export the director caontaining the model and all its associated files from colab to a local directory. 
...
Once the isnatllation completes, run the following command to convert the model. Note that the correct path to the model (with a '.pb' suffix) must be provided. 
