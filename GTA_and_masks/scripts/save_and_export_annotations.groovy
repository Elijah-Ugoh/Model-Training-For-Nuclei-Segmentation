...
def saveImages(def images, def labels, def name) {
    ...
    def target_folder = new File ( buildFilePath('/Users/Elijah/Documents/BINP37_Research_Project/GTA_and_masks/images_and_masks', 'ground_truth', 'masks' ) )
    mkdirs( source_folder.getAbsolutePath() )
    mkdirs( target_folder.getAbsolutePath() )
    
    IJ.save( images , new File ( source_folder, name ).getAbsolutePath())
    IJ.save( labels , new File ( target_folder, name ).getAbsolutePath())
